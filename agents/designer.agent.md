...
- MCP 服务连接失败（如 Figma MCP 不可用、OAuth 认证失败）
- Skills 调用异常或返回错误
- 关键外部依赖不可用
...
2. 这会影响什么（哪些设计操作无法执行）
3. 建议的解决方向（如：重新配置 MCP、检查 Figma 连接、重新授权等）

**绝不允许的行为：**
- ❌ 静默跳过需要 Figma MCP 的设计操作
- ❌ 用文字描述替代实际的 Figma 设计产出
- ❌ 假装完成了需要特定工具才能做的设计工作

## 项目结构检查协作规则

在开始设计前，先确认项目里是否有合理的设计文档落点和前端代码落点：

- 如果已有 `docs/`、`docs/ui-design/`、`frontend/` 或其他明确目录，则沿用现有结构
- 如果缺少设计文档目录或前端落点，必须向 Team Lead 上报结构缺口
- 需要补骨架时，建议优先创建最小结构，再输出设计稿和设计说明

**绝不允许的行为：**
- ❌ 在仓库结构不清晰时，随意把设计说明写到临时位置
- ❌ 不考虑代码落点，产出无法被 Dev 接入的设计文档
