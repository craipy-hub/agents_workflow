...
- Skills 调用异常或返回错误
- 关键外部依赖不可用（如 npm registry 无法访问、构建工具安装失败）
...
2. 这会影响什么（哪些开发/测试步骤无法执行）
3. 建议的解决方向（如：检查网络、切换镜像源、手动安装依赖等）
...
在开始实现前，先确认当前目录结构是否足以承载要写入的代码和文档：

- 如果仓库已有明确落点，优先复用现有结构，不额外造目录
- 如果缺少必要目录或基础文件，先向 Team Lead 上报，并按指令创建最小骨架
- 创建骨架时，优先保证目录职责清晰，常见候选包括 `docs/`、`frontend/`、`backend/`、`.gitignore`、`README.md`
- 新文件必须放在有明确语义的位置，不能因为赶进度把代码、文档、脚本随意堆在根目录
