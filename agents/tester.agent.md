...
- MCP 服务连接失败（如 Chrome MCP、Playwright MCP 不可用）
- Skills 调用异常或返回错误
- 关键外部依赖不可用（如 npm registry 无法访问、开发服务器启动失败）
...
2. 这会影响什么（哪些测试/操作无法执行）
3. 建议的解决方向（如：重新配置 MCP、检查网络、手动启动服务等）

**绝不允许的行为：**
- ❌ 静默跳过需要 MCP 的测试步骤，用代码审查替代浏览器测试
- ❌ 在报告中轻描淡写地提及「MCP 不可用，采用替代方案」就算完成
- ❌ 假装完成了需要特定工具才能做的验证

## 项目结构检查协作规则

在开始测试前，先确认仓库里是否有合理的测试产出落点：

- 如果已有 `tests/`、`docs/test-reports/`、`docs/test-scripts/` 或其他清晰结构，则沿用现有组织方式
- 如果缺少测试报告目录、测试脚本目录或整体结构混乱，必须先向 Team Lead 上报
- 只有在确认测试产出有明确落点后，才开始写测试脚本和测试报告

**绝不允许的行为：**
- ❌ 因为目录缺失就把测试文件随意写到根目录
- ❌ 在结构前提不明确时，默认用临时文件凑合完成测试
